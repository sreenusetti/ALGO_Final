...
def analyze_cpr_strategy(indicators, pivot_data, ai_predictor, ohlc_df=None):
    ...
    return {
        ...
        "cpr_width": cpr_width,
        "cpr_levels": {
            "TC": tc, "BC": bc, "R1": r1, "R2": r2, "R3": r3,
            "S1": s1, "S2": s2, "S3": s3, "PDH": pdh, "PDL": pdl,
...
class OrderManager:
    ...
    # Signal mode for entries
    SIGNAL_MODE = "both"

    def __init__(self, fyers_service, symbol, lot_size, log_fn, state_path, ai_predictor: CPR_AIPredictor, event_log=None, bot=None, **_):
    ...
    def _process_entry(self, side, reason, ltp, atr, bar_key=None, indsP=None):
        """
        Handles live entries and simulates entries on 'margin shortfall' in PAPER_TRADING_MODE.
        """
        indsP = indsP or {}
        ...
        self.log(f"[_process_entry] Checking entry: side={side}, reason={reason}, ltp={ltp}, atr={atr}", True)
        ...
        if is_live_success or is_paper_override:
            ...
            self.log(f"{'PAPER ' if is_paper_override else ''}ENTRY SUCCESS: {side} {self.symbol}. "
            ...
            self._append_trade_csv({
                ...
                "bar_key": bar_key,
                "adx": self._f(indsP.get("adx")), "macd_color": indsP.get("macd_color"),
                "ema5": self._f(indsP.get("ema_5")), "ema9": self._f(indsP.get("ema_9")), "ema21": self._f(indsP.get("ema_21"))
            })
        ...
        self._append_trade_csv({
            "trade_id": f"{self.symbol}-{int(time.time())}", "symbol": self.symbol, "side": side, "event": "ENTRY_FAIL",
            "entry_time": self._now_iso(), "entry_ltp": ltp, "reason": msg, "order_id": (resp or {}).get("id"),
            "bar_key": bar_key,
            "adx": self._f(indsP.get("adx")), "macd_color": indsP.get("macd_color"),
            "ema5": self._f(indsP.get("ema_5")), "ema9": self._f(indsP.get("ema_9")), "ema21": self._f(indsP.get("ema_21"))
        })
    ...
    def execute_unified_strategy(self, ltp, all_inds, primary_tf=None):
        ...
        inds = _tf(primary_tf)
        if not isinstance(inds, dict) or not inds.get("timestamp"):
            self.log(f"[UNIFIED] No indicators for TF={primary_tf}.", True)
            return
        ...
        # ==========================================
        # DETECT TREND (Enhanced)
        # ==========================================

        detected_trend = None
        try:
            df_5 = self.bot.fetch_ohlc(self.symbol, "5", 30) if hasattr(self, 'bot') else None
            ...
            self.log(f"[TREND-DETECT] Error: {e}", True)
            detected_trend = None
        ...
        signals = {
            "trend": None,  # Combined EMA+MACD
            "ai_cpr": None,  # AI ML model
            "cpr_strategy": None,  # CPR pivot strategy
            "macd": None,  # MACD momentum
            "ema_cross": None,  # EMA crossover
            "price_action_trend": None  # 🔥 NEW: Enhanced trend detection
        }
        ...
        if is_choppy:
            self.log(f"[UNIFIED] Choppy market detected (BBW: {bb_bandwidth:.4f}) - blocking entries", True)
            if current_pos != "FLAT":
                atr_here = self._get_atr_with_fallback(inds, ltp)
                if atr_here:
                    self._update_trailing_and_breakeven(ltp, atr_here)
        ...
        if macd_color == "Dark Green" and adx_strong:
            signals["macd"] = "BUY"
            confidences["macd"] = 0.7
        elif macd_color == "Dark Red" and adx_strong:
            signals["macd"] = "SELL"
            confidences["macd"] = 0.7
        ...
        # 4️⃣ 🔥 NEW: PRICE ACTION TREND SIGNAL (from enhanced detect_trend)
        if detected_trend:
            trend_lower = detected_trend.lower()
            ...
            elif "consol" in trend_lower:
                # Consolidation - don't add to voting, but log it
                self.log(f"[PRICE-ACTION] Consolidation detected - no directional bias", True)
        ...
        try:
            # Get pivot data for AI predictor
            pivot_data = inds.get("cpr_analysis", {})
            if not pivot_data or "TC" not in pivot_data:
                # Fallback: load from file if not in indicators
                try:
                    pivot_json_path = self.state_path.replace("om_state", "pivot")
                    pivot_data = robust_load_json(
                        pivot_json_path,
                        self.log,
                        default={}
                    ).get(self.symbol, {})
            ...
            # 🔥 CALL AI PREDICTOR DIRECTLY FOR REAL-TIME DECISION
            ...
            ai_confidence_raw = ai_conf if ai_conf is not None else 0.0

            self.log(
                f"[AI-CPR] Real-time prediction: {ai_label} (confidence: {ai_confidence_raw:.3f})",
                False
            )

            # Map AI prediction to trading signal
        ...
        # 6️⃣ CPR STRATEGY SIGNAL (from pre-calculated analysis)
        cpr_analysis = inds.get("cpr_analysis", {})
        ...
        # ==========================================
        # DECISION LOGIC WITH AI PRIORITY
        # ==========================================

        final_signal = None
        reason = ""

        # 🔥 PRIORITY 1: High confidence AI signal (≥0.75) - can override others
        ...
        if ai_in_buy and confidences["ai_cpr"] >= 0.75:
            final_signal = "BUY"
            reason = f"HIGH CONFIDENCE AI BUY ({confidences['ai_cpr']:.2f}) [Other votes: {buy_count - 1}]"

        elif ai_in_sell and confidences["ai_cpr"] >= 0.75:
            final_signal = "SELL"
            reason = f"HIGH CONFIDENCE AI SELL ({confidences['ai_cpr']:.2f}) [Other votes: {sell_count - 1}]"
        ...
        # 🔥 PRIORITY 3: Medium confluence (2 votes with AI or high score)
        elif buy_count == 2:
            if ai_in_buy or buy_score >= 1.3:
                final_signal = "BUY"
                ai_part = f" (AI: {confidences['ai_cpr']:.2f})" if ai_in_buy else ""
                reason = f"Medium BUY confluence (2/6 votes, score: {buy_score:.2f}){ai_part}"

        elif sell_count == 2:
            if ai_in_sell or sell_score >= 1.3:
                final_signal = "SELL"
                ai_part = f" (AI: {confidences['ai_cpr']:.2f})" if ai_in_sell else ""
                reason = f"Medium SELL confluence (2/6 votes, score: {sell_score:.2f}){ai_part}"
        ...
        if final_signal and current_pos == "FLAT":
            ...
            if atr_here:
                ...
                if self._process_entry(final_signal, full_reason, ltp, atr_here, bar_key=bar_key, indsP=inds):
                    ...
                    self._save_state()
                    self.log(f"[UNIFIED] ✅ Entry executed: {final_signal}", False)
                    return
        ...
        # 🔥 LAYER 1: CPR REVERSAL AT MAJOR LEVELS (Highest Priority)
        if current_pos == "BUY" and reversal_at_resistance:
            # Check for bearish candle patterns at resistance
            bearish_signals = []
            if candle_patterns.get("big_bear_takeout"):
                bearish_signals.append("Big Bear Take Out")
            if candle_patterns.get("fake_bull"):
        ...
        if current_pos == "BUY":
            if ai_in_sell and confidences["ai_cpr"] >= 0.70:
                ...
                self.log(f"[AI-EXIT] {reason}", False)
                self._process_exit(reason, ltp)
                self._set_cooldown(1)
                self._save_state()
                return
    ...
    def detect_trend(self, df, i=-1):
        ...
        try:
            ...
            if 'high' in df.columns and 'low' in df.columns:
                ...
                # If current range < 50% of average, it's consolidating
                consolidating = current_range < avg_range * 0.5
            else:
                consolidating = False
