...
class OrderManager:
    ...
    def execute_unified_strategy(self, ltp, all_inds, primary_tf=None):
            ...
            if current_pos != "FLAT":
                ...
                try:
                    ...
                    if atr_here:
                        ...
                        # Try to get pivot data for dynamic SL
                        pivot_data = inds.get("cpr_analysis", {}) if isinstance(inds, dict) else {}
                        if pivot_data:
        ...
        signals = {
            ...
            "price_action_trend": None  # Enhanced trend detection
        }
        ...
        # 4️⃣ PRICE ACTION TREND SIGNAL (from enhanced detect_trend)
        if detected_trend:
        ...
        try:
            # Get pivot data for AI predictor with validation
            pivot_data = inds.get("cpr_analysis", {})
            ...
            # Validate pivot data before AI prediction
        ...
        # Validate CPR analysis
        if not cpr_analysis or not isinstance(cpr_analysis, dict):
            self.log("[CPR-STRATEGY] CPR analysis missing - signal skipped", True)
        else:
            # Check if CPR levels are valid
            cpr_valid = bool(
                cpr_analysis.get("TC") and
                cpr_analysis.get("BC") and
                cpr_analysis.get("R1") and
                cpr_analysis.get("S1")
            )
        ...
        # Check if AI is in the votes
        ai_in_buy = any(s == "ai_cpr" for s, _ in buy_votes)
        ai_in_sell = any(s == "ai_cpr" for s, _ in sell_votes)

        # 🔥 PRIORITY 1: High confidence AI signal (≥0.75) WITH OPPOSITION CHECK
        if ai_in_buy and confidences["ai_cpr"] >= 0.75:
            opposing_signals = sell_count
            supporting_signals = buy_count - 1  # Exclude AI itself

            # Allow if:
            # 1. At least 1 other agrees, OR
            # 2. Opposition is weak (≤2 opposing)
        ...
        # 🔥 PRIORITY 3: Medium confluence (2 votes) WITH TIGHTENED THRESHOLDS
        elif buy_count == 2:
            # Require BOTH AI involvement AND decent score, OR very high score
            ...
            elif buy_score >= 1.5:  # Two very strong votes (e.g., Trend 0.80 + MACD 0.70)
                final_signal = "BUY"
        ...
        # ==========================================
        # EXIT LOGIC - MULTIPLE LAYERS (VALIDATED)
        # ==========================================

        # Extract CPR reversal data with validation
        ...
        if current_pos != "FLAT":
            ...
            if atr_here:
                ...
                if cpr_analysis and isinstance(cpr_analysis, dict):
                    self._update_dynamic_cpr_stop_loss(ltp, atr_here, cpr_analysis)
            self._check_trailing_profit(ltp, inds)
        self._save_state()
